...
- [ ] 3.4 更新安装器进度回调以使用新的进度条
- [x] 3.5 使用不同终端宽度测试进度条
- [x] 3.6 确保进度条在小屏幕上正确显示

## 4. ANSI 颜色和主题
- [ ] 4.1 在 `src/ui/colors.rs` 中创建颜色常量和主题系统
- [ ] 4.2 实现彩色输出函数（成功、错误、警告、信息、进度）
- [ ] 4.3 实现颜色检测和禁用逻辑
- [ ] 4.4 在 `src/cli/args.rs` 中添加 `--no-color` 标志到 CLI 选项
- [ ] 4.5 确保颜色在各种终端模拟器上工作
- [ ] 4.6 测试带和不带标志的颜色输出
...
## 6. 配置系统增强
- [ ] 6.1 向配置文件格式添加 `shell` 配置选项
- [ ] 6.2 实现从配置中解析自定义 shell 命令
- [ ] 6.3 更新登录过程以使用自定义 shell 命令
- [ ] 6.4 修改 `src/config.rs` 以支持新的配置选项
- [ ] 6.5 为自定义 shell 命令添加验证
- [ ] 6.6 使用各种命令格式测试自定义 shell
